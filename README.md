...
4. **播放端 H.265 兼容性验证**：
   默认配置保持 H.264，所有设备、音频、编码和传输参数集中在 `config/visioncast_config.json`。两个运行脚本按脚本名选择摄像头，RTP/WebRTC/RTMP 均可按 `encoder.video_codec` 选择 H.264 或 H.265。后续重点是补充不同浏览器、播放器和 RTMP 服务端对 H.265/Enhanced RTMP 的兼容性记录。
