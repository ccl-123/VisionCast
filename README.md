...
4. **播放端 H.265 兼容性验证**：
   默认配置保持 H.264，RTP/WebRTC/RTMP 均可按 `encoder.video_codec` 选择 H.264 或 H.265。完整 H.265 示例为 `config/visioncast_config_h265.json`，运行脚本可通过 `VISIONCAST_CONFIG=config/visioncast_config_h265.json` 选择。后续重点是补充不同浏览器、播放器和 RTMP 服务端对 H.265/Enhanced RTMP 的兼容性记录。
