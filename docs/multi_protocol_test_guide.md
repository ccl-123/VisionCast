...
- 默认配置为 H.264 RTMP；若切换到 H.265，可使用完整配置 `config/visioncast_config_h265.json`，播放端和服务端需要支持 Enhanced RTMP/HEVC。
- 脚本测试 H.265 RTMP 示例：
  ```bash
  VISIONCAST_CONFIG=config/visioncast_config_h265.json ./install/visioncast/scripts/run/run_13855_camera.sh rtmp
  ```
- **使用 ffplay 拉流播放**：
  ```bash
...
```
若 `encoder.video_codec` 配置为 `h265`，或通过 `VISIONCAST_CONFIG=config/visioncast_config_h265.json` 启动，视频 SDP 会改为 `a=rtpmap:96 H265/90000` 并写入 H.265 fmtp。
**跨设备播放步骤**：
