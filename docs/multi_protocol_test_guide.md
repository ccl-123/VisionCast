...
本指南详细介绍了如何在 RK3588 开发板端及同局域网跨设备上，对 VisionCast 系统的四大流媒体传输协议（WebRTC WHIP/WHEP、RTMP、RTSP、RTP over UDP）进行闭环推拉流测试。本教程将作为后续开发和系统集成测试的重要参考。
...
### 2.3 RTSP 模式
VisionCast 的 RTSP 模式**不依赖外部流媒体服务器**，其通过集成静态 RTSP 服务端库 `librtsp.a` 直接在系统内部监听端口，提供直接被拉流的能力。

#### 2.3.1 推流（服务开启）命令
启动 RTSP 推流模式，程序会在 `8555` 端口（或配置文件指定端口）自动创建 RTSP 服务实例并等待客户端接入：
```bash
env LD_LIBRARY_PATH=./install/visioncast/lib ./install/visioncast/bin/visioncast --protocol rtsp
```

#### 2.3.2 播放拉流命令
- **在本机播放拉流**（需要在开发板上启动桌面播放器）：
  ```bash
  ffplay rtsp://127.0.0.1:8555/live
  ```
- **跨设备播放拉流**（在局域网 PC 上）：
  ```bash
  ffplay rtsp://192.168.137.202:8555/live
  ```
