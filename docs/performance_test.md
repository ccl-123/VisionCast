...
预览不会影响后续协议发送的数据正确性，因为 RTP/RTMP/WebRTC/RTSP 使用的是已经生成的 `EncodedPacket`。但显示端不是全链路零拷贝：X11 需要 `NV12 DMA-BUF -> RGA RGB/BGRA -> XPutImage`，Framebuffer 需要 `NV12 DMA-BUF -> RGA -> mmap fb`，因此开启预览会增加少量 RGA/内存带宽和显示提交开销。
...
- RTSP：Annex-B NALU 解析用于提取 H.264/H.265 参数集，视频和 Opus 音频交给内置 FFmpeg 8.1.1 RTSP muxer 写出。
- RTP/WebRTC/RTSP 音频：Opus 输入 PCM 缓存使用 offset 消费，并复用 Opus 输入 scratch buffer，避免每帧前端 `erase` 和临时 PCM vector 分配。
...
- `发送`：`AvTransport::send_audio()` 本地调用耗时。RTP/WebRTC/RTSP 包含 Opus 编码和协议封包；RTMP 包含推给 FFmpeg RTMP 队列。
