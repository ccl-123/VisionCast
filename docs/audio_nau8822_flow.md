...
- **采集时钟**：ALSA PCM 读取完成后记录 `CLOCK_MONOTONIC` 时间，并按实际返回采样帧数回推一个 buffer 时长，使 `AudioFrame::pts_us` 表示这段 PCM 的第一个采样点时间。
- **编码器实现**：
