...
> 音频输入：NAU8822 `hw:1,0`
> 传输协议：WebRTC WHIP、RTSP、RTMP、RTP over UDP (均已实现)
...
VisionCast 总体链路分为视频链路、音频链路、本地显示链路和网络推流链路。当前版本已经把摄像头到 MPP 视频编码器的主路径改为 DMA-BUF/硬件缓冲传递，协议封包仍保留必要的 CPU 可见编码包拷贝。本地预览在视频编码成功返回后异步分支，使用同一个 `VideoFrame` 的 DMA-BUF 句柄，不复制主路径 NV12。
...
[RgaProcessor] (MJPEG 解码、RGA fd/VA、直通或 CPU fallback)
     │
     ▼
[MppEncoder] (MPP H.264/H.265 硬编码，优先 EXT_DMA 输入)
     │
...
[AvTransport] (多协议推送抽象层)
     ...
     ├─> [RtspPusher] (Annex-B/Opus -> FFmpeg RTSP)
     ├─> [RtmpPusher] (Annex-B -> FFmpeg FLV/Enhanced RTMP)
...
```text
V4L2 EXPBUF fd -> VideoFrame::dma -> RGA 直通 -> MPP EXT_DMA 输入 -> H.264/H.265
```
...
- **声道处理**：`audio.capture_channels` 控制 ALSA 实际采集声道数，`audio.channels` 控制推流输出声道数。两者一致时直接透传；配置为双声道采集、单声道输出时，`AudioCapture` 在底层进行左右声道均值混音。
...
- **编码格式**：由主配置 `encoder.video_codec` 选择 `h264` 或 `h265`，默认保持 H.264。摄像头参数、音频参数、编码参数和传输参数全部集中在 `config/visioncast_config.json`。
...
- **协议边界**：RTP/WebRTC/RTSP/RTMP 均可按配置使用 H.264 或 H.265；WebRTC H.265 依赖播放端能力，RTMP H.265 依赖支持 Enhanced RTMP/HEVC 的服务端和播放器。
...
音频按传输协议采用两条明确路径：
1. **Opus**：RTP、WebRTC 与 RTSP 共用 `AudioEncoder`，使用 48 kHz RTP 时钟、动态负载类型 `111` 和带内 FEC。
...
- **物理实现**：链接项目内置 FFmpeg 8.1.1 开启 `whip` 封装器建立网络连接，客户端通过 HTTP POST 向 WHIP 服务端推送 SDP Offer，服务器返回 SDP Answer。
- **DTLS与ICE建连优化**：在 webrtc_pusher 中配置 `whip_flags` 为 `dtls_active`，强制推流端作为 DTLS 主动握手发起端（Active 角色），实现毫秒级握手连接；同时，利用编译期补丁逻辑在解析 SDP Answer 时，优先筛选与 WHIP URL 宿主机 IP 完全相同的 Candidate 实施 UDP 绑定，有效规避了 Clash VPN 虚拟网卡等物理网卡多网卡环境下的建连挂起问题。
- **音视频数据链路**：
  - **视频链路**：`13855 MIPI CSI 采集 (/dev/video11)` -> `DMA-BUF NV12 直通或RGA fd处理` -> `MppEncoder H.264/H.265硬编码` -> `FFmpeg whip 封装器以SRTP发送` -> `主机MediaMTX接收` -> `WHEP 播放端`。
  - **音频链路**：`nau8822 hw:1,0 采集 (48kHz S16_LE)` -> `AudioCapture输出 PCM` -> `AudioEncoder编码为Opus` -> `FFmpeg whip 封装器以SRTP加密发送` -> `主机MediaMTX` -> `浏览器 WHEP 播放`。
- **数据压缩规格**：
  - **视频**：H.264 或 H.265，完全禁用 B 帧，CBR 控制，GOP=30。浏览器端 H.265 解码能力依赖系统和浏览器支持。
  - **音频**：Opus，RTP 时钟 48000Hz，20ms 帧，目标码率 64 kbps，启用带内 FEC。

### 9.2 RTMP
- **物理实现**：链接项目内置 FFmpeg 8.1.1 (`libavformat.so.62` 等) 建立 RTMP 传输层通道，向指定的 RTMP 服务端发布流。
- **字节流封装**：`RtmpPusher` 从首个 Annex-B 视频帧提取 H.264 SPS/PPS 或 H.265 VPS/SPS/PPS 作为 extradata，随后将 MPP 输出的 Annex-B packet 交给 FFmpeg FLV muxer。新版 muxer 会写出传统 AVC FLV tag 或 Enhanced RTMP/HEVC tag。
- **音视频数据链路**：
  - **视频链路**：`13855 MIPI CSI 采集` -> `DMA-BUF NV12 直通或RGA fd处理` -> `MppEncoder H.264/H.265编码` -> `RtmpPusher` -> `FFmpeg FLV/Enhanced RTMP封装` -> `RTMP over TCP推送` -> `主机MediaMTX` -> `播放器拉流`。
  - **音频链路**：`nau8822 hw:1,0 采集` -> `AudioCapture按配置输出 PCM` -> `FFmpeg重采样并软编码为AAC-LC` -> `FLV容器封装` -> `RTMP over TCP推送` -> `主机MediaMTX` -> `ffplay拉流`。
- **数据压缩规格**：
  - **视频**：H.264 或 H.265；H.265 RTMP 使用 Enhanced RTMP/HEVC (`hvc1`) 封装。
  - **音频**：AAC-LC (Advanced Audio Coding Low Complexity)，采样率 48000Hz，声道数跟随 `audio.channels`，16-bit 深度。

### 9.3 RTSP
- **物理实现**：链接项目内置 FFmpeg 8.1.1 开启 `rtsp` muxer，向指定 RTSP 服务端执行 ANNOUNCE/RECORD 发布。`stream.rtsp_transport` 支持 `udp` 或 `tcp`，默认 `udp`。
- **字节流封装**：`RtspPusher` 从首个 Annex-B 视频帧提取 H.264 SPS/PPS 或 H.265 VPS/SPS/PPS 作为 extradata，由 FFmpeg 生成 SDP 并通过 RTP muxer 写出媒体包。
- **音视频数据链路**：
  - **视频链路**：`13855 MIPI CSI 采集` -> `DMA-BUF NV12 直通或RGA fd处理` -> `MppEncoder H.264/H.265编码` -> `RtspPusher` -> `FFmpeg RTSP/RTP封装` -> `主机MediaMTX` -> `播放器拉流`。
  - **音频链路**：`nau8822 hw:1,0 采集` -> `AudioCapture输出 PCM` -> `AudioEncoder编码为Opus` -> `FFmpeg RTSP/RTP封装` -> `主机MediaMTX` -> `ffplay/VLC拉流`。
- **数据压缩规格**：
  - **视频**：H.264 或 H.265。
  - **音频**：Opus，RTP 时钟 48000Hz，声道数跟随 `audio.channels`。
...
- **音视频数据链路**：
  - **视频链路**：`13855 MIPI CSI 采集` -> `DMA-BUF NV12 直通或RGA fd处理` -> `MppEncoder H.264/H.265编码` -> `RtpPacketizer封包(RFC 6184/RFC 7798分片)` -> `UDP 发送(端口5004)` -> `主机 ffplay(解析sdp)播放`。
  - **音频链路**：`nau8822 hw:1,0 采集` -> `PCM` -> `AudioEncoder编码为Opus` -> `RtpPacketizer封装(Payload Type 111)` -> `UDP 发送(端口5006)` -> `主机 ffplay(解析sdp)播放`。
- **数据压缩规格**：
  - **视频**：H.264 或 H.265，RTP 动态负载类型 `96`。
...
5. **音频采集线程 (Audio Capture Thread)**
   ...
   - **职责**：专职通过 ALSA API 定期从音频输入端 `hw:1,0` 抓取 20ms 周期的原始 PCM 数据，并按 `audio.capture_channels`/`audio.channels` 转换后推入音频管线原始帧队列 `raw_queue_` 中。
6. **音频处理与编码线程 (Audio Pipeline Thread)**
   ...
   - **职责**：从音频队列中消费 PCM 数据并交给 `AvTransport`；RTP/WebRTC/RTSP 由共享 `AudioEncoder` 编码为 Opus，RTMP 由 `RtmpPusher` 编码为 AAC。
7. **第三方协议栈后台网络线程 (Third-Party Network Threads)**
   - 在 WebRTC 传输模式下，FFmpeg 底层网络协议栈和 SSL (OpenSSL) 会在底层启动线程或利用 I/O 复用，来独立进行：
     - **ICE 连接状态与 STUN 绑定包发送**；
     - **DTLS/SRTP 加密和数据交互**。
...
1. **编码后 packet view 化**：
   当前已收敛 RTP/WebRTC 逐包发送、RTP H.264/H.265 流式 NALU 扫描、RTMP/RTSP 参数集提取和音频 PCM offset 消费。剩余主要拷贝是 MPP 输出写入 `EncodedPacket::data`、RTP 最终 packet payload、WebRTC/SRTP 内部发送，以及 FFmpeg RTMP/RTSP 封装内部写出。
2. **动态码率与拥塞控制 (Congestion Control)**：
   目前推流仅使用固定的 CBR 码率。未来可引入 WebRTC/RTCP 反馈机制，读取客户端返回的丢包率和 RTT 延迟，实时动态调整 MPP 编码器的 `Bitrate` 目标。
3. **Opus 自适应优化**：
   当前 RTP/WebRTC/RTSP 已使用 Opus。后续可依据 RTCP 反馈动态调整码率、FEC 和预期丢包率。
4. **H.265 播放兼容性验证**：
   H.265 已接入 MPP、RTP、WebRTC、RTSP 与 RTMP。后续需要补充不同浏览器、播放器和服务端对 H.265 的兼容性记录。
