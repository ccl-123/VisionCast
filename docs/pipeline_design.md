...
- **协议边界**：RTP/WebRTC/RTMP 均可按配置使用 H.264 或 H.265；RTMP H.265 依赖支持 Enhanced RTMP/HEVC 的服务端和播放器。
...
1. **编码后 packet view 化**：
   当前已收敛 RTP/WebRTC 逐包发送、RTP H.264/H.265 流式 NALU 扫描、RTMP 参数集提取和音频 PCM offset 消费。剩余主要拷贝是 MPP 输出写入 `EncodedPacket::data`、RTP 最终 packet payload、WebRTC/SRTP 内部发送，以及 FFmpeg RTMP 封装内部写出。
...
4. **H.265 播放兼容性验证**：
   H.265 已接入 MPP、RTP、WebRTC 与 RTMP。后续需要补充不同浏览器、播放器和 RTMP 服务端对 H.265/Enhanced RTMP 的兼容性记录。
