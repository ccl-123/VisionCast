 ...
 * 它集成了 UDP/RTP 传输、RTMP 推流、WebRTC 推流以及 RTSP 推流功能，并根据系统配置分发音视频流。
 * 
 * 主要职责包括：
 * 1. 统一管理各种底层发送器（UDP, RTMP, WebRTC, RTSP）的初始化与销毁。
...
public:
    ...
    /**
     * @brief 打开并初始化所有配置的传输通道（如 UDP 发送器、RTMP/WebRTC/RTSP 推流器等）
...
private:
    ...
    AudioEncoder audio_encoder_;     ///< RTP/WebRTC/RTSP 共用的 Opus 音频编码器
    RtmpPusher rtmp_;               ///< RTMP 推流器组件，负责向 RTMP 服务器推送音视频流
    WebRtcPusher webrtc_;           ///< WebRTC 推流器组件，负责低延迟音视频互动流传输
    RtspPusher rtsp_;               ///< RTSP 推流器组件，负责向 RTSP 服务器推送音视频流
